...
class Traffic_Dataset():
	...
	def __init__(self):
		...
		with open("drive/Colab/grab/Traffic_management/traffic_image_fake.txt", "rb") as fp:
			saved_data = pickle.load(fp)
		images=saved_data[0]

		with open("drive/Colab/grab/Traffic_management/traffic_timing_fake.txt", "rb") as fp:
			saved_timing = pickle.load(fp)

		self.timing=saved_timing[0]
	...
	def __getitem__(self,index):
		...
		for i in range(self.time_sequence):
			num_images.append(np.array(self.x_data[index+i]))
			num_time.append(self.timing[index+i])
			if i>(self.time_sequence-2):
				num_target.append(self.x_data[index+i+1])
		#num_time=torch.from_numpy(np.asarray(num_time))
		#num_time=torch.zeros(len(num_time), 96).scatter_(1, num_time.unsqueeze(1), 1.)
		num_target=torch.from_numpy(np.array(num_target, dtype='float'))
		return torch.from_numpy(np.asarray(num_images)),num_target,torch.from_numpy(np.asarray(num_time))
	...
	def read_data(self):
		...
		df=pd.read_csv(filepath_or_buffer = "drive/Colab/grab/Traffic_management/training.csv")
